...
Plot_Size_withMeanSEMandpvalues <- function(
  ...
  PrimaryProtien_Number_and_Association_Events <- paste0(
    Size_Summary_ByCohort$SHORT_LABEL[1], " - PRIMARY_PROTIEN_TRACK_COUNT_byCOHORT = ", Size_Summary_ByCohort$PRIMARY_PROTIEN_TRACK_COUNT_byCOHORT[1], " \n",
    " and PRIMARY_PROTIEN_ASSOCIATION_EVENT_COUNT_byCOHORT = ", Size_Summary_ByCohort$PRIMARY_PROTIEN_ASSOCIATION_EVENT_COUNT_byCOHORT[1], " \n", 
    "PERCENTAGE_MEAN = ", signif(Size_Summary_ByCohort$MEAN_OF_MEAN_PERCENTAGE_OF_TRACKS_LARGER_THAN_THRESHOLD[1], digits = 3), " +/- SEM = ", signif(Size_Summary_ByCohort$SEM[1], digits = 3) 
  )  
  for(index in 2:nrow(Size_Summary_ByCohort)){
    PrimaryProtien_Number_and_Association_Events <- paste0(
      PrimaryProtien_Number_and_Association_Events, " \n", " \n", 
      Size_Summary_ByCohort$SHORT_LABEL[index], " - PRIMARY_PROTIEN_TRACK_COUNT_byCOHORT = ", Size_Summary_ByCohort$PRIMARY_PROTIEN_TRACK_COUNT_byCOHORT[index], " \n",
      " and PRIMARY_PROTIEN_ASSOCIATION_EVENT_COUNT_byCOHORT = ", Size_Summary_ByCohort$PRIMARY_PROTIEN_ASSOCIATION_EVENT_COUNT_byCOHORT[index], " \n", 
      "PERCENTAGE_MEAN = ", signif(Size_Summary_ByCohort$MEAN_OF_MEAN_PERCENTAGE_OF_TRACKS_LARGER_THAN_THRESHOLD[index], digits = 3), " +/- SEM = ", signif(Size_Summary_ByCohort$SEM[index], digits = 3)
    )
  }
