...
Table2 <- fread("/Users/u_niranjan/Desktop/Git Scripts/01_IRAK4 Phosphorylation Paper/00_Cohort_table/05_3xKO+MyD88-3xFLAG-IRES-Puro+mIRAK4WT-GFP+mIRAK1DD-mScarlet_Analysis.csv.gz")

# Setting directory paths
Plot_Directory_Save_Path <- "/Users/u_niranjan/Desktop/Cell_Figures/Plots"
Plot_Script_Directory <- "/Users/u_niranjan/Desktop/Git Scripts/01_IRAK4 Phosphorylation Paper/IRAK4_Autophosphorylaytion_Paper/02_Figure_Scripts/01_Figure1/Figure_1I_1J"
...
Table <- Table %>% 
  mutate(
    PROTEIN = case_when(
      PROTEIN == "IRAK1WT" | PROTEIN == "IRAK1DD" ~ "IRAK1",
      PROTEIN == "IRAK4WT" ~ "IRAK4" 
    ),
    SHORT_LABEL = case_when(
      SHORT_LABEL == "IRAK4 WT + IRAK1 WT" ~ "WT",
      SHORT_LABEL == "IRAK4 WT + IRAK1 DD" ~ "IRAK4WT\n+\nIRAK1DD",
      TRUE ~ SHORT_LABEL
    )
  ) %>% 
  as.data.table()
...
# Creating Folder to save Images
Plot_Directory_Path <- file.path(Plot_Directory_Path, "Figure_1I_1J")
if(!file.exists(Plot_Directory_Path)){
  dir.create(Plot_Directory_Path)
}
