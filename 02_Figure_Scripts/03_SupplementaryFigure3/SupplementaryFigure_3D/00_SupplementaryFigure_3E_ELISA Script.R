...
Input_Table <- fread("/Users/u_niranjan/Desktop/Cell_Figures/00_New_Figures/Supplementary Figure 3/ELISA - MyD88-GFP_IRAK4-mScarlet/TablestoAnalyse.csv")
...
Plot_Directory_Path <- file.path(Plot_Directory_Save_Path, "03_SupplementaryFigure3", "SupplementaryFigure_3G")
...
ELISA_Fx <- function(Plate_Count) {
  ...
  Plate_Standards <- Plate[Stimulation_Condition == "Calibration"] %>%
    ...
    summarise(Values_Measured_mean = mean(Values_Measured)) %>%
    mutate(Treatment_Condition = as.numeric(Treatment_Condition)) %>%
  ...
  Calibration_Plot <- ggplot(data = Plate_Standards, aes(x = Values_Measured_mean, y = Treatment_Condition)) +
    geom_point(size = 5) +
    geom_line(aes(y = Fit_Test), linetype = "dashed") +
    annotate('text', x = 0.15, y = 700, label = paste0("R^2 = ", signif(Rsquare, 4)), size = 10) +
    annotate('text',
    ...
    labs(x = "Measured Values", y = "IL-Concentration (pg/mL)", title = paste("Plate", Plate_Count, "Standard Curve")) +
...
cohort_order_base <- c("DMSO", "500nM", "20uM")

# Process the combined data.
Processed_Table <- SplitPlateTable %>%
  # Filter out any irrelevant conditions.
  filter(Treatment_Condition != "IRAK4KI_NoTreatment") %>%
  mutate(
    ...
    Treatment_Condition_short = case_when(
      Treatment_Condition == "IRAK4KI+DMSO"  ~ "DMSO",
      Treatment_Condition == "IRAK4KI+500nM" ~ "500nM",
      Treatment_Condition == "IRAK4KI+20uM"  ~ "20uM"
    ),
...
Processed_Table <- Processed_Table %>%
  ...
  arrange(Treatment_Condition_short_Extended)
