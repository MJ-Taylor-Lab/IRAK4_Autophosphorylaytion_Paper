Plot_Directory_Save_Path_Sub_Dir <- file.path(Plot_Directory_Path, "SupplementaryFigure4A")
...
Table_filtered <- Table %>% 
  filter(
    IMAGE == "20230608 plate001_well3E_5nM_cl085_MyD88_IRAK4KI_notrat_001"
  )
...
for(index in 1:nrow(Dwell_Test_List)){
  ...
  Number_of_Cells <- unique(Colocalisation_Table_Split[, .(CELL, COLOR)])
  
  # Write csv ---------------------------------------------------------------
  Plot_Save_Path <- file.path(Plot_Directory_Save_Path_Sub_Dir, "00_Time_Step_Table.csv")
  write.csv(Colocalisation_Table_Split, Plot_Save_Path)
  
  Plot_Save_Path <- file.path(Plot_Directory_Save_Path_Sub_Dir, "00_Cell_List.csv")
  write.csv(Number_of_Cells, Plot_Save_Path)
  ...
  Plot <- ggplot(
    ...
    scale_x_continuous(
      ...
      breaks = seq(0,max(Colocalisation_Table_Split$TIME_STEP)+1, by = 300),
      expand = c(0,0)
    ) +
    scale_y_continuous(
      n.breaks = 3
