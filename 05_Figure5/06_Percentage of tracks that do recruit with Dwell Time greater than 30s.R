...
Plot_Directory_Save_Path_Sub_Dir <- file.path(Plot_Directory_Save_Path, "03_Percentage of tracks that do recruit with Dwell time above 30s")
...
# Creating a list of tracks that are over 30s
Cell_Summary_by_Track_List_GreterThan30s <- Cell_Summary_by_Track_List %>% 
  filter(
    DWELL_TIME >= 30
  ) %>% 
  distinct(
...
Cell_Summary_by_Track <- Table %>% 
  ...
  filter(
    RECRUITING_TRACK_TEST == "TRUE"
  ) %>% 
  mutate(
    DWELL_TIME_SPOT_TEST = UNIVERSAL_TRACK_ID %in% Cell_Summary_by_Track_List_GreterThan30s$UNIVERSAL_TRACK_ID,
...
geom_segment(
  ...
  scale_y_continuous(
    limits = c(0,111),#c(0,100),
    breaks = seq(0,111, by = 10)#seq(0,100, by = 10)
  ) +
  labs(
    x = "Treatment",
...
# Cleanup -----------------------------------------------------------------
rm(
  Cell_Summary_by_Track_List_GreterThan30s,
