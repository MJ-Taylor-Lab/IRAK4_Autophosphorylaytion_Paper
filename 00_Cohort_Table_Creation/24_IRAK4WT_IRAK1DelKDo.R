...
Table1 <- fread("/Volumes/TAYLOR-LAB/Niranjan/04 Image Analysis/Analysis Output/20221022/24_IRAK4WT_IRAK1DelKDo/20231130/Output/Analysis.csv.gz")
...
Table <- Table %>% 
  filter(
    COHORT == "TKO+MyD88-3xFLAG+IRAK4WT+IRAK1DelKDo"
  ) %>% 
  mutate(
    SHORT_LABEL = "IRAK4 WT + IRAK1 DelKDo"
  ) %>% 
  group_by(
    UNIVERSAL_TRACK_ID
  ) %>% 
...
Table_path <- "/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/24_IRAK4WT+IRAK1DelKDo_Compiled_Essential.csv.gz"
