...
Table1 <- fread("/Volumes/TAYLOR-LAB/Niranjan/04 Image Analysis/Analysis Output/20221022/16_IRAK3WildType/20231010/Output/Analysis.csv.gz")
...
Table <- Table %>% 
  filter(
    COHORT == "IRAK3WildType"
  ) %>% 
  mutate(
    SHORT_LABEL = "IRAK3 WT"
  ) %>% 
  group_by(
    UNIVERSAL_TRACK_ID
  ) %>% 
...
Table_path <- "/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/16_IRAK3WT_Compiled_Essential.csv.gz"
