...
Colocalisation_Percentage_byCell <- read_excel(Primary_Data, sheet = "Supplementary Figure 4H")
...
# Creating Folder to save Images
Plot_Directory_Save_Path <- file.path(Plot_Directory_Save_Path, "Supplementary Figure 4H")
if(!file.exists(Plot_Directory_Save_Path)){
  dir.create(Plot_Directory_Save_Path)
}
...
Colocalisation_Percentage_byImage <- Colocalisation_Percentage_byImage %>% 
  mutate(
    JITTER_COLOR = case_when(
      SHORT_LABEL == "R12C" ~ "#F99108",  # Assign colors based on SHORT_LABEL
      SHORT_LABEL == "KD" ~ "#F99108",
