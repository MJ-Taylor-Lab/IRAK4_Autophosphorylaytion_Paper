...
Colocalisation_Percentage_byCell <- read_excel(Primary_Data, sheet = "Supplementary Figure 3E")
...
# Creating Folder to save Images
Plot_Directory_Save_Path <- file.path(Plot_Directory_Save_Path, "Supplementary Figure_3E")
if(!file.exists(Plot_Directory_Save_Path)){
  dir.create(Plot_Directory_Save_Path)
}
...
Colocalisation_Percentage_byImage <- Colocalisation_Percentage_byImage %>% 
  mutate(
    JITTER_COLOR = case_when(
      SHORT_LABEL == "DMSO" ~ "#33AFCC",
      SHORT_LABEL == "KI" ~ "#F99108"
    )
  )
