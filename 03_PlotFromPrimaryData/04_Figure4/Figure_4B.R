...
Cell_Summary_by_Cohort <- read_excel(Primary_Data, sheet = "Figure 4B")
...
# Creating Folder to save Images
Plot_Directory_Save_Path <- file.path(Plot_Directory_Save_Path, "Figure_4B")
if(!file.exists(Plot_Directory_Save_Path)){
  dir.create(Plot_Directory_Save_Path)
}
...
BarPlot +
  ...
  annotate(
    ...
    x = 15,
    y = 1,
    label = Cell_Summary_by_Cohort$Combination_Name_Full[4]
  )
