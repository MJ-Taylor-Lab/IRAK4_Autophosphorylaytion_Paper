...
Input_Table <- fread("/Users/u_niranjan/Desktop/ELISA/20231101/20231101_5/TablestoAnalyse.csv")
Output_Directory <- "/Users/u_niranjan/Desktop/ELISA/20231101/20231101_5"

Dilution_Factor <- 5

Cohort_Order_Table_Path <- "/Users/u_niranjan/Desktop/ELISA/20231101/20231101_5/CohortOrder.csv" #Put nonsense string if you dont want order 
...
SplitPlateTable <- SplitPlateTable %>% 
  ...
  mutate(
    ...
    Treatment_Condition_short = Treatment_Condition,
...
# Statistics --------------------------------------------------------------
#Pull Out Cohort List
Treatment_Condition_list <- unique(PlateSummarybyDay$Treatment_Condition_short) %>%  as.data.table()
colnames(Treatment_Condition_list) <- c("Treatment_Condition_short")

#Create matrix using Cohort list as row and columns
Staistics_matrix <- matrix(data = NA, nrow = nrow(Treatment_Condition_list)+1, ncol = nrow(Treatment_Condition_list)+1)
for(col_index in 1:nrow(Treatment_Condition_list)){
  Staistics_matrix[1, col_index+1] = Treatment_Condition_list$Treatment_Condition_short[col_index]
}

for(row_index in 1:nrow(Treatment_Condition_list)){
  Staistics_matrix[row_index+1, 1] = Treatment_Condition_list$Treatment_Condition_short[row_index]
}

#Do statistical tests to fill up matrix
for(col_index in 2:ncol(Staistics_matrix)){
  for(row_index in 2:nrow(Staistics_matrix)){
    if(row_index != col_index){
      PlateSummarybyDay_temp <- PlateSummarybyDay %>%
        filter(
          Treatment_Condition_short == Staistics_matrix[1, col_index] | Treatment_Condition_short == Staistics_matrix[row_index, 1]
        )
      Stat <- aov(Unstimulated_by_Stimulated ~ Treatment_Condition_short, data = PlateSummarybyDay_temp) #One Way ANOVA 
      Stat <- summary(Stat)
      Stat <- Stat[[1]]$`Pr(>F)`[1]
      Stat <- signif(Stat, digits = 5)
      Staistics_matrix[row_index, col_index] = as.numeric(Stat)
    } else {
      Staistics_matrix[row_index, col_index] = 1
    }
  }
}

Statistics_Save <- file.path(Output_Directory, "05_Staistics.csv")
write.csv(Staistics_matrix, Statistics_Save, row.names = FALSE)

rm(
  col_index,
  row_index,
  PlateSummarybyDay_temp,
  Stat,
  Staistics_matrix,
  Cohort_Order_Table,
  Treatment_Condition_list
)
...
PlateSummarybyTreatment_Condition_CellLine <- PlateSummarybyTreatment_Condition %>% 
  mutate(
    COLOR = case_when(
      Treatment_Condition_short == "MyD88-GFP" ~ "dark green",  #yellowish orange,
      Treatment_Condition_short == "IRAK4 KO" ~ "red",
      Treatment_Condition_short == "IRAK4 WT" ~ "orange",# greenish lightblue
      Treatment_Condition_short == "IRAK4 KD" ~ "lightblue",
      Treatment_Condition_short == "IRAK4 DD" ~ "blue"
    )
  )
