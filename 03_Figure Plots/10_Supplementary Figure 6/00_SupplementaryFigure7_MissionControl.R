...
Table1 <- fread("/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/11_IRAK1WT_Compiled_Essential.csv.gz")
Table2 <- fread("/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/12_IRAK2WT_Compiled_Essential.csv.gz")
Table3 <- fread("/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/16_IRAK3WT_Compiled_Essential.csv.gz")


Plot_Directory_Save_Path <- "/Users/u_niranjan/Desktop/Possible paper figure/Git Figure_20231102" # Where to save Images
Plot_Script_Directory <- "/Users/u_niranjan/Desktop/Git Scripts/IRAK4_Autophosphorylaytion_Paper/03_Figure Plots/10_Supplementary Figure 6" #Where Plot Scripts are are
...
Table <- Table %>% 
  mutate(
    ORDER_NUMBER = case_when(
      SHORT_LABEL == "IRAK1 WT" ~ 1,
      SHORT_LABEL == "IRAK2 WT" ~ 2,
      SHORT_LABEL == "IRAK3 WT" ~ 3
    )
  ) %>% 
  as.data.table()

# Creating Folder to save Images
Plot_Directory_Path <- file.path(Plot_Directory_Save_Path, "07_SupplementaryFigure 7")
...
Plot_Script_Sub_Directory <- file.path(Plot_Script_Directory, "07_Figure7")
