...
Plot_Directory_Save_Path_Sub_Dir <- file.path(Plot_Directory_Save_Path, "01_Max Normalised Intensity MyD88 Lifetime Greater Than 50s")
...
Cell_Summary_by_Track$SHORT_LABEL <- 
  factor(
    Cell_Summary_by_Track$SHORT_LABEL,
    levels = c("IRAK1 WT", "IRAK2 WT", "IRAK3 WT")
  )
...
Cell_Summary_by_Cohort <- Cell_Summary_by_Track %>% 
  ...
  mutate(
    ...
    COLOR  = case_when(
      SHORT_LABEL == "IRAK1 WT" ~ "brown",
      SHORT_LABEL == "IRAK2 WT" ~ "orange",
      SHORT_LABEL == "IRAK3 WT" ~ "coral1"
    )
  ) %>% 
  arrange(
    ORDER_NUMBER,
...
UPPER_LIMIT = 16       #Axis Upper Limit
...
Plot +
  ...
  annotate(
    ...
    y = 200,
    label = paste0(
      "IRAK1 WT Mean +/- SEM = ", signif(Cell_Summary_by_Cohort$MAX_NORMALIZED_INTENSITY_MEAN[1], digits = 4), " +/- ", signif(Cell_Summary_by_Cohort$SEM[1], digits = 4), "\n",
      "IRAK2 WT Mean +/- SEM = ", signif(Cell_Summary_by_Cohort$MAX_NORMALIZED_INTENSITY_MEAN[2], digits = 4), " +/- ", signif(Cell_Summary_by_Cohort$SEM[2], digits = 4), "\n",
      "IRAK3 WT Mean +/- SEM = ", signif(Cell_Summary_by_Cohort$MAX_NORMALIZED_INTENSITY_MEAN[3], digits = 4), " +/- ", signif(Cell_Summary_by_Cohort$SEM[3], digits = 4)
    ),
    color="black",
    size=3
  ) +
...
Plot_Save_Path_1 <- "01_Max Normalized Intensity MyD88 Histogram with mean and SEM.pdf"
...
Plot_Save_Path_1 <- "02_Max Normalized Intensity MyD88 Histogram without mean, SEM and no facet labels.pdf"
Plot_Save_Path <- file.path(Plot_Directory_Save_Path_Sub_Dir, Plot_Save_Path_1)
ggsave(
  Plot_Save_Path,
  plot = last_plot(),
