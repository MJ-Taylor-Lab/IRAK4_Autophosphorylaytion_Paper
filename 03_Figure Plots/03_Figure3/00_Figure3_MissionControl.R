...
Table1 <- fread("/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/03_IRAK4KI_DMSO_Compiled_Essential.csv.gz")
Table2 <- fread("/Users/u_niranjan/Desktop/Git Scripts/00_Myddosomal_internal_phosphorylation_cohort_table/04_IRAK4KI_PF06650833_20uM_Compiled_Essential.csv.gz")

Plot_Directory_Save_Path <- "/Users/u_niranjan/Desktop/Possible paper figure/Git Figure_20231213" # Where to save Images
Plot_Script_Directory <- "/Users/u_niranjan/Desktop/Git Scripts/IRAK4_Autophosphorylaytion_Paper/03_Figure Plots/03_Figure3" #Where Plot Scripts are are
...
Table <- Table %>% 
  mutate(
    ORDER_NUMBER = case_when(
      SHORT_LABEL == "DMSO" ~ 1,
      SHORT_LABEL == "Kinase Inhibitor 20 uM" ~ 2
    )
  ) %>% 
  as.data.table()
