...
Cell_Summary_by_Track$SHORT_LABEL <- 
  factor(
    Cell_Summary_by_Track$SHORT_LABEL,
    levels = c("DMSO", "Kinase Inhibitor 20 uM")
  )
...
UPPER_LIMIT = 36       #Axis Upper Limit
...
Plot +
  ...
  annotate(
    ...
    label = paste0(
      ...
      "Kinase Inhibitor 20 uM Mean +/- SEM = ", signif(Cell_Summary_by_Cohort$MAX_NORMALIZED_INTENSITY_MEAN[2], digits = 4), " +/- ", signif(Cell_Summary_by_Cohort$SEM[2], digits = 4)
    ),
    color="black",
    size=3
  ) +
