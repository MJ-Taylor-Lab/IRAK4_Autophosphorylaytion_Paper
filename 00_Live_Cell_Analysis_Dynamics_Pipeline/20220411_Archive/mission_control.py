...
if not platform.system() == 'Darwin':
    ...
    parameter_tables = '/Users/u_deliz/Desktop/20211214/Input/parameter_tables'
    use_x_cpus = 8
...
try:
    ...
    n_ligand_images = len(ligand_list) - 1
...
try:
    ...
    n_protein_images = len(median_remove_image_list) - 1
...
try:
    ...
    ray.init(num_cpus=use_x_cpus)
    result_ids = []
    n_segmentation_images = len(segmentation_image_list) - 1
    ...
    ray.init(num_cpus=use_x_cpus)
    result_ids = []
    n_segmentation_images = len(substack_image_list) - 1
    n_segmentation_images = range(0, n_segmentation_images)
    for image_x in n_segmentation_images:
...
# For relaying the intensities from the xml, extrapolate the location of the puncta
from subprocess import call
