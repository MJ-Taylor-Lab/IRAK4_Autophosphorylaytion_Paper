    ...
    try:
        ...
        for image_x in n_images:
            try:
                ...
                if os.path.exists(channels_csv_path) and os.path.exists(area_csv_path) and os.path.exists(
                        metadata_csv_path):
                    # Get tables
                    ...
                    for channel_x in range(0, len(channel_metadata)):
                        try:
                            ...
                            trackmate_threshold = float(trackmate_threshold)
                            ...
                            for cell_x in range(0, len(cells)):
                                try:
                                    ...
                                    print('tracking_list cell loop error')
                                    pass
                            ...
                            print('tracking_list channel error')
                            pass
                ...
                print('tracking_list image loop error')
                pass
        # Concatenate
...
def tracking_parameters(all_channels_metadata, file_ending, segmentation_path):
    try:
        ...
        experimental_table = experimental_table[['protein_name', 'protein_relative_path',
                                                 'trackmate_threshold', 'trackmate_frame_gap',
                                                 'trackmate_gap_link_distance', 'trackmate_max_link_distance']]
        experimental_table['protein_name'] = 'Combined'

        experimental_table = experimental_table.assign(
            protein_relative_path=lambda dataframe: dataframe['protein_relative_path'].map(
                lambda path: os.path.dirname(path))
        )
        experimental_table = experimental_table.assign(
            protein_relative_path=lambda dataframe: dataframe['protein_relative_path'].map(
                lambda path: os.path.join(path, 'Combined'))
        )
        experimental_table = experimental_table.drop_duplicates()

        all_channels_metadata = experimental_table.append(calibration_table)
        all_channels_metadata = all_channels_metadata.reset_index()
        ...
        print('tracking_parameters error')
        pass