...
y_variables <-c(
  ...
  "FRAMES_ADJUSTED", "N",
  "DELTA_RATIO", "ADJUSTED_DELTA_RATIO"
)
...
PlotFx <- function(FacetX){
  tryCatch({
    ...
    InterpolationTable <-
      ...
      mutate(
        x_axis = round(x_axis/FINAL_STEP_SIZE),
        x_axis = as.integer(x_axis)
      ) %>% 
      complete(
        x_axis = full_seq(c(min(x_axis), max(x_axis)), period = 1/21),
      ) %>%
      fill(
        one_of(
          "y_axis",
      ...
      mutate(
        FINAL_STEP_SIZE = min(FINAL_STEP_SIZE, na.rm = T),
        x_axis = x_axis*FINAL_STEP_SIZE,
        y_axis_old = y_axis,
