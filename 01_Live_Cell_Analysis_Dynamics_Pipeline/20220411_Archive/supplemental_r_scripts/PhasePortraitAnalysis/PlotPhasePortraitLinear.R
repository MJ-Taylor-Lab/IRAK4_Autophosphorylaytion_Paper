    ...
    StreamTempTable$d_qry_int[is.na(StreamTempTable$d_qry_int)] = 0
      ...
      ggplot(
        ...
        geom_streamline(
          ...
          # arrow.length = 0.3,
          # jitter = 4,
          L = 250, res = 20, xwrap = c(0, 360), lineend = "round"
        ) +
        scale_size(range = c(.01, 1), guide = "none") +
        scale_alpha(guide = "none") +
        scale_mag(
...
# mclapply(1:NROW(PlotList), StreamPlotFx)
