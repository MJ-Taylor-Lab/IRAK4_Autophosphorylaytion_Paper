...
if(grepl("macOS", osVersion)){
  # Table <- fread(file = "/Users/u_deliz/Desktop/Grid Data/Essential.csv.gz", fill=TRUE)
  # Table <- fread(file.path(TABLE_PATH, "Essential.csv.gz"), fill=TRUE)
  Table <- fread("~/TRAF6.csv.gz", fill=TRUE)
} else{
  library(R.utils)
...
ScaleFx <- function(TableX){
  ...
  TempTable <-
    TableX %>% 
    mutate(
      MIN_REFERENCE = quantile(REFERENCE_TOTAL_INTENSITY, LOWER_BOUNDARY, na.rm = T),
      MIN_QUERY = quantile(QUERY_TOTAL_INTENSITY, LOWER_BOUNDARY, na.rm = T),
      
      MAX_REFERENCE = quantile(REFERENCE_TOTAL_INTENSITY, UPPER_BOUNDARY, na.rm = T),
      MAX_QUERY = quantile(QUERY_TOTAL_INTENSITY, UPPER_BOUNDARY, na.rm = T)
    ) %>%
    mutate(
