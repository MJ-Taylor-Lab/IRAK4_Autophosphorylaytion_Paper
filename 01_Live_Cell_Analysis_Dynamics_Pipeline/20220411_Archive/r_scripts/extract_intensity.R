...
pacman::p_load(dplyr, parallel, tidyr, data.table, ff, ijtiff, XML)
...
XMLtoTableFx <- function(FileX){
  tryCatch({
    ...
    filter_name = tools::file_path_sans_ext(xml_path)
    remove_n_char = dirname(dirname(dirname(dirname(filter_name))))
    filter_name <- substr(filter_name, nchar(remove_n_char)+2, nchar(filter_name))
    # Get cell parameters
    temp_file_list <-
      file_list %>% 
      filter(
        protein_relative_path == filter_name
      )
    # Get cell parameters
    # Unsure why line below was written
    # cell_parameters = temp_file_list[FileX,]
    if(NROW(temp_file_list)>1){
      print("temp_file_list has one than one parameter")
    }
    cell_parameters <- as_tibble(temp_file_list)
    # Rename columns
    ...
    # Remove if it exists
    file.remove(save_path, showWarnings = TRUE)
    ...
    if(NROW(MissingSpotsTables)>0 |cell_parameters$COHORT != "Calibrations"){
      ...
      MissingSpotsTables <- mclapply(MissingSpotsTables, MissingSpotsFx, mc.cores = detectCores(logical = F))
    ...
    FrameFx <- function(TableX){
      tryCatch({
        ...
        SubPixelLocalization <- function(SpotX){
          ...
          Result$SPOT_AREA = sum(TotalIntensity$SPOT_AREA) - sum(is.na(TotalIntensity$TOTAL_INTENSITY))
          Result$TOTAL_INTENSITY = sum(TotalIntensity$TOTAL_INTENSITY, na.rm = TRUE)
          Result$STANDARD_DEVIATION = sd(TotalIntensity$TOTAL_INTENSITY, na.rm = TRUE)
    ...
    ShortFullTable <- as.data.frame(ShortFullTable)
    Intensities <- as.data.frame(Intensities)
    CompleteIntensities <- merge(ShortFullTable, Intensities, by = "UNIVERSAL_SPOT_ID", all = T)
