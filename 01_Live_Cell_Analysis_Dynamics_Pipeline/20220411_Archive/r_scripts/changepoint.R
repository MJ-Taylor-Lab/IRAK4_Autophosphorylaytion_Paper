...
# Library list
pacman::p_load(dplyr, stringr, parallel, tidyr, data.table, changepoint, ff, R.utils) 
...
EvaluateTable <- function(TableX){
  tryCatch({
  print(paste("TableX =", TableX, "of", nTables))
  # Get paths
  ...
  if(NROW(FilteredTable) > 0){
    # Run changepoint
    CPResults <- mclapply(FilteredTable, ChangepointFx, mc.cores = detectCores(logical = F))
